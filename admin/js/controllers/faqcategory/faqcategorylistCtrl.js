'use strict';
/* Controllers */
app.controller('faqcategorylistCtrl', ['$scope', '$window', '$http', '$rootScope', '$timeout', '$filter', '$stateParams', '$state','toaster','prompt',
    function ($scope, $window, $http, $rootScope, $timeout, $filter, $stateParams, $state,toaster,prompt) {
        $scope.id = localStorage.getItem('aid');
        $scope.rowCollection = [];
        $scope.type = "faq";

        /** GET FAQ FOLDER BY TYPE*/
        function getData() {
            var paramd = {
                eType:$scope.type
            };
