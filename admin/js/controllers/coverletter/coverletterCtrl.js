'use strict';
/* Controllers */
app.controller('coverlistCtrl', ['$scope', '$window', '$http', '$rootScope', '$timeout', '$filter', '$stateParams', '$state','toaster','prompt',
    function ($scope, $window, $http, $rootScope, $timeout, $filter, $stateParams, $state,toaster,prompt) {
        ...
        /** GET COVERLETTER DATA*/
        function getData() {
            $http.get(webservice_path +'coverletter').success(function (res) {
                if (res.Status == 'True') {
        ...
        $scope.onBlur = function($event,pid,prio) {            
            ...
            if(paramd.prio != undefined){
                $http.post(webservice_path + 'coverletter/changePriority', paramd).success(function (res) {
                    if (res.Status == 'True') {
        ...
        $scope.removeItem = function (id, catName) {
            ...
            prompt({
                ...
                $http.post(webservice_path + 'coverletter/delete',param).success(function (res) {
        ...
        $scope.changestatus = function (id, eStatus) {
            ...
            prompt({
                ...
                if(name.label == 'Yes'){
                    $http.post(webservice_path + 'coverletter/changeStatus',param).success(function (res) {
                        if (res.StatusCode == '999'){
                            $scope.logout();
                            toaster.pop('error','',res.Message);
                        }else if (res.Status == 'True'){
        ...
        $scope.checkSelection = function (){
            ...
            angular.forEach($scope.rowCollection, function (value, key) {
                if (document.getElementById('cat' + value.iPkCoverLetterId)) {
                    if (document.getElementById('cat' + value.iPkCoverLetterId).checked == true) {
                        suc = 1;
                    }
                }
            });
            ...
            if (selectcondition == 1){
                ...
                prompt({
                    ...
                    angular.forEach($scope.rowCollection, function (value, key) {
                        if (document.getElementById('cat' + value.iPkCoverLetterId)){
                            if (document.getElementById('cat' + value.iPkCoverLetterId).checked == true){
                                allrecords.push(value.iPkCoverLetterId);
                            }
                        }
                    });
                    var param = {
                    ...
                    $http.post(webservice_path + 'coverletter/deleteAll',param).success(function (res) {
                        if (res.StatusCode == '999'){
                            $scope.logout();
                            toaster.pop('error','',res.Message);
                        }else if (res.Status == 'True') {
        ...
        $scope.activeAll = function () {
            ...
            if (selectcondition == 1) {
                ...
                prompt({
                    ...
                    $http.post(webservice_path + 'coverletter/changeStatusAll',param).success(function (res) {
                        if (res.StatusCode == '999'){
                            $scope.logout();
                            toaster.pop('error','',res.Message);
                        }else if (res.Status == 'True') {
